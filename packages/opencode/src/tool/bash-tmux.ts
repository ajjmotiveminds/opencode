/**
 * Tmux-backed bash tool
 * 
 * Executes bash commands inside the app container's tmux session.
 * Provides same UX as BashTool but with human-visible execution in tmux.
 */

import z from "zod/v4"
import { Tool } from "./tool"
import { Instance } from "../project/instance"
import { TmuxExecutor } from "../exec/tmux-executor"
...
const log = Log.create({ service: "bash-tmux-tool" })
...
export const BashTmuxTool = Tool.define("bash_tmux", {
  description: DESCRIPTION + "\n\nThis version executes commands in a tmux session for visibility.",
  parameters: z.object({
  ...
  async execute(params, ctx) {
    ...
    log.info("Executing command via tmux", { 
      command: params.command, 
      sessionID: ctx.sessionID,
      socket: tmuxSocket,
      session: tmuxSession,
